...
@if(!empty($offerProductData) && count($offerProductData) > 0)
@php
    $last_id = '';
    $i=0
@endphp
@foreach ($offerProductData as $key2 => $result )
@php
    $product_title='';
    ...
    <div class="bs-box">
        ...
        <div class="bs-content">
            ...
            {{-- <a title="{{Helper::language('add_to_cart')}}" href="{{route('productdetails',['id'=>Helper::encodeUrl($result->product_id)])}}" class="add-bucket"><i class="icon-bucket"></i></a> --}}
             ...
             @if ($available_qty>0)
                <a style="display: <?php echo $adisplay ; ?>;" title="{{Helper::language('add_to_cart')}}"  data-product-id="{{ Helper::encodeUrl($result->product_id) }}"  data-variant-id="{{ $result->id }}" data-offer_status="{{ $result->offer_status}}"  class="add-bucket"  href="javascript:void(0);"><i class="icon-bucket"></i></a>
            @endif

            <input class="favs-icon checked_box" type="checkbox" id="fav-item-offer{{$key2}}" value="{{ ($fav_data != "") ? '1' : "0" }}" {{ ($fav_data != "") ? 'checked' : "" }} onclick="return productFav({{$result->product_id}},{{ ($fav_data != "") ? '1' : "0" }})" />
            <label  class="favs-button" title="{{Helper::language('add_to_favourite')}}" for="fav-item-offer{{$key2}}"></label>
        </div>                          
        <div class="load_offer_products_ids"  data-id="{{Helper::encodeUrl($result->product_id)}}" >  </div>
    </div>
</div>
@php
    $i++;
@endphp
@if(count($offerProductData)==$i) 
<input type="hidden" @if(!empty($product_last_id)) value="{{$product_last_id}}" @else value="{{$result->product_id}}" @endif id="offer-last-id" >
@php
...
@endphp
<div id="appen-offer-html{{$pd_id}}" ></div>
@endif
@endforeach
@else
...
<input type="hidden" id="offerProFcount" value="{{@$offer_showing_product_count?:''}}">
<input type="hidden" id="offerProTcount" value="{{@$offer_total_product_count?:''}}">
</div>
