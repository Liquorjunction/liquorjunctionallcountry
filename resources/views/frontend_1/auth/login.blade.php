        ...
        <div id="Register">
            <div class="registration-content">
                ...
                <div class="registration-card">
                    <form action="#" class="row registration-form" method="POST" enctype="multipart/form-data" id="login_form">
                        <div class="col-12">
                            <div class="form-group">
                             
                                <label for="">{{@Helper::language('email_label')}} / {{@Helper::language('phone_number')}}<span class="valid_field">*</span></label>
                                <input type="text" placeholder="{{@Helper::language('enter_email_place')}} / {{@Helper::language('phone_number')}}" name="email" id="email">
                        ...
                        <div class="col-12">
                            <div class="form-group form-password">
                                ...
                                <input type="password" name="password" placeholder="{{@Helper::language('enter_password_placeholder')}}" required>
                                ...
                                <span class="help-block" id="errorMessagePassword" style="display:none;">
                                    <span class="mb-4"style="color: #FF4444;font-size:14px; display: none; float:left;" id="errorMsgPassword" class='validate validate-error text-center'></span>
                                </span>
                                <div class="forget-pass">
                                    <a href="{{ route('userforgotpassword') }}">{{@Helper::language('forgot_password_text')}}</a>
                                </div>
                            </div>
                        </div>
                        <span class="help-block" id="errorMessagecommon" style="display:none">
                            <span style="color: #FF4444;font-size:14px;display: none;" id="errorMsgcommon" class='validate validate-error text-center'></span>
                        </span>
                        <span class="help-block" id="errorMessageEmail" style="display:none">
                            <span style="color: #FF4444;font-size:14px;display: none;" id="errorMsgEmail" class='validate validate-error text-center'></span>
                        </span>
                        <div id="otp_verify"></div>
                        <div class="col-12">

                            <button type="submit" class="solid-button w-100">{{@Helper::language('login_label')}}</button>
                            ...
                            <p class="text-center">{{@Helper::language('login_not_register_yet')}}<a href="{{ route('websiteregister') }}">{{@Helper::language('sign_up_btn')}}</a></p>
                        </div>
                    </form>
                </div>
            </div>
...
<script type="text/javascript">
    ...
    var these_credentials_do_not_match_our_records = "{{ Helper::language('these_credentials_do_not_match_our_records'); }}";  
        ...
        submitHandler: function() {
            ...
            if (checkout_value == 1) {
                ...
                // console.log(pack_size);
                    ...
                    var checkoutUrl = '{{ route("checkout") }}'; // This fetches the route '/checkout'
    ...
    // window.location.href = checkoutUrl;
                ...
                error: function(errors) {
                    ...
                    var b = JSON.parse(a);
                    // alert(b.email);
                   if(b.email == "These credentials do not match our records" || b.email == "Ces informations d'identification ne correspondent pas à nos dossiers")
                   {
                    ...
                    if (erroJson.status == "error" || erroJson.id != "" ) {
                        ...
                        if(erroJson.id){
                            $("span#errorMsgcommon").html(erroJson.errors+' <a href="'+url+'">click here</a>');
                        }else{
                            $("span#errorMsgcommon").html(erroJson.errors);    
