...
class OrderController extends Controller
{
    ...
    public function anyData(Request $request)
    {
        ...
        $user_type_filter = $request->get('user_type_filter');
        $payment_type_filter = $request->get('payment_type_filter');
        $payment_status_filter = $request->get('payment_status_filter');
        ...
        $totalAr = Order::join('main_users', 'main_users.id', '=', 'order.user_id')
            ...
            ->leftjoin('countries', 'countries.id', '=', 'order_info.country_id')
            ->rightjoin('transactions', 'transactions.order_id', '=', 'order.id')
            ->select('order.*', 'order_info.customer_name as customer_name', 'order_info.customer_mobile as customer_mobile','order_info.promocode_name' ,'countries.name as country_name', 'transactions.payment_type', 'transactions.payment_status as payment_status', 'order_status.name as delivery_status', 'main_users.is_guest_user')
        ...
        if (!empty($from_date)) {
            $totalAr->whereDate('order.order_date', '>=', $from_date);
        }
        if (!empty($to_date)) {
            $totalAr->whereDate('order.order_date', '<=', $to_date);
        }
        if ($user_type_filter != "") {
            $totalAr->where('main_users.is_guest_user', $user_type_filter);
        }
        if (!empty($payment_type_filter)) {
            $totalAr->where('transactions.payment_type', $payment_type_filter);
        }
        if (!empty($payment_status_filter)) {
            $totalAr->where(function($q) use ($payment_status_filter) {
                if ($payment_status_filter == 1) { // Pending
                    $q->where(function($q1) {
                        $q1->where('transactions.payment_type', '!=', 3)
                           ->whereNotIn('transactions.payment_status', [1, 2]);
                    })->orWhere(function($q2) {
                        $q2->where('transactions.payment_type', 3)
                           ->where('order.order_status', '!=', 3)
                           ->where('transactions.payment_status', 1);
                    });
                } elseif ($payment_status_filter == 2) { // Success
                    $q->where(function($q1) {
                        $q1->where('transactions.payment_type', '!=', 3)
                           ->where('transactions.payment_status', 1);
                    })->orWhere(function($q2) {
                        $q2->where('transactions.payment_type', 3)
                           ->where(function($q3) {
                               $q3->where('transactions.payment_status', 2)
                                  ->orWhere('order.order_status', 3);
                           });
                    });
                } elseif ($payment_status_filter == 3) { // Failed
                    $q->where(function($q1) {
                        $q1->where('transactions.payment_type', '!=', 3)
                           ->where('transactions.payment_status', 2);
                    })->orWhere(function($q2) {
                        $q2->where('transactions.payment_type', 3)
                           ->where('order.order_status', '!=', 3)
                           ->whereNotIn('transactions.payment_status', [1, 2]);
        ...
        if (!empty($delivery_status)) {
            $totalAr->where('order.order_status', $delivery_status);
        }
        ...
        ';
            ...
            $data_arr[] = array(
                ...
                "user_type" => $data->is_guest_user ? '<span class="badge badge-warning">Guest</span>' : '<span class="badge badge-success">Register</span>',
