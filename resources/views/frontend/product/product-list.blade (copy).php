...
<div class="loader" id="loader"></div> 
    ...
    ?>
    <!-- Title Banner -->
    <section class="title-banner" style="background-image: url({{asset('assets/frontend/images/title-banner.png')}}); background-repeat: no-repeat; background-size: cover;">
    <div class="container">
        <div class="row">
            <div class="col-12">
                @if (!empty($subCategoryDetails))
                    @php
                        if (session::get('language') == 1) {
                            ...
                            $sub_title = ($subCategoryDetails->title_fr) ? $subCategoryDetails->title_fr : $subCategoryDetails->title;                            
                        }
                    @endphp
            ...
            <nav aria-label="breadcrumb">
                <ul class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">
                        <a  @if (!empty($subCategoryDetails))
                            href="{{ route('productlist', ['id' => Helper::encodeUrl($categoryDetails->id)]) }}" @endif>
                            {{ $category_title }}
                        </a>
                    </li>
                    @if (!empty($subCategoryDetails))
    ...
    <section class="product-listing">
        <div class="container">
            <div class="row" id = "product_list">
                <div class="col-12">
                    <div class="short-by">
                        <p class="mb-0">{{ Helper::language('all_products') }} ( <span id="total_page_count">{{ @$total_product_count ?: '0' }}</span> Items )</p>
                    ...
                    <div class="row">
                        <div class="offset-md-5 col-md-7">
                            <div class="show-more">                                
                                <button  @if ($total_product_count!=0 && $total_product_count > 16) style="display:block;" @else style="display:none;" @endif class="solid-button show-more load-more-data">{{ Helper::language('show_more') }}</button>
                                @if ($total_product_count!=0)
                                <p id="showing">{{ Helper::language('showing') }}
                                    {{-- <span id="first_page_count"></span> to --}}
                                    <span id="p-current-count">{{$showing_product_count?:0}} </span> of <span id="p-total-count">{{$total_product_count}}</span>
                                </p>
                                @endif
                            </div>
                        </div>
        ...
        <div class="offcanvas-body">
                                ...
                                <li>
                                    <div class="check-group">
                                        <input class="form-check-input product_categories" name="product_categories[]"
                                            ...
                                            @if ($page_category_id == $categoty_result->id) {{ 'checked' }} @endif
                                            id="category_id_{{ $key }}">
                                        <label class="form-check-label"
                                            for="category_id_{{ $key }}">{{ $category_title }}</label>
                                    </div>
                                    @if ($categoty_result->subcategory && $categoty_result->subcategory != null)
                                        <ul class="sub-filter-list" @if(count($categoty_result->subcategory) == 1) style="overflow-y: inherit" @endif >
            ...
            <div class="filter-button">
                <div class="filter-button-group">
                    ...
                    <a href="javascript:void(0)" id="apply-filter" class="solid-button " data-bs-dismiss="offcanvas" aria-label="Close">{{ Helper::language('apply_filter') }}</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <script type="text/javascript">        
        ...
        function getProductData() {
            ...
            var max_price = $("#max-price").val();
            $.ajax({
                url: "{{ route('productFilterData') }}?brand_ids=" + brand_ids +
