...
class SubadminController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $check_view_permission = @Helper::GetRolePermission(Auth::user()->user_type,24,'read');
    ...
    public function index()
    {
        ...
        return view("dashboard.subadmin.list",compact('countries'));
    }
    ...
    public function edit(Request $request)
    {
        ...
        {   
            ...
            {
                $html =  view('dashboard.subadmin.edit')->with(['customerData' => $customerData])->render();
        ...
        if (empty($request->customer_id)) {
            $validator = \Validator::make($request->all(), 
                    [
                        ...
                        'email' =>  ['required',
                                    'regex:/(.+)@(.+)\.(.+)/i','email',
                                    Rule::unique('users')->where(function ($query){ return $query->where('status','!=','2');
                                    })],
                    ...
                    [
                        ...
                        'fullname.max' => 'The name must not be grater than 40 characters',
                        'phone.required' => 'The phone number field is required',
                        'profile.required' => "The profile photo field is required",
                        'profile.max'=>'The profile photo should be less than 2 MB .',
                        'profile.mimes'=>'The profile photo must be jpeg, png, jpg',
            ...
            $subadmin->status = 1;
            $subadmin->user_type = 3;
            $subadmin->parent_id = Auth::user()->id;
            $subadmin->save();
            $ismail = $this->send_email_subadmin($request->email,$password,$request->fullname);
            Alert::success('Success', __('backend.New_Subadmin_created_successfully'));
            ...
            $validator = \Validator::make($request->all(), 
                    [
                        ...
                        'email' =>  ['required',
                                    'regex:/(.+)@(.+)\.(.+)/i','email',
                                    Rule::unique('users')->ignore($customer_id)->where(function ($query){
            ...
            Alert::success('Success', __('backend.Subadmin_has_been_updated_successfully'));
            return response()->json(['success' => 'true']);
        }
    }

    public function send_email_subadmin($email,$password,$fullname) {
    ...
    public function show(Request $request)
    {
        ...
        if(!empty($customerData))
        {
            $html =  view('dashboard.subadmin.show')->with(['customerData' => $customerData])->render();
            return response()->json(['success' => true,'html'=> $html]);
        }
        return response()->json(['success' => false,'msg'=> 'something wrong.']);
    }
    ...
    public function status_active(Request $request){
        $customer_id = $request->customer_id;
         ...
         Alert::success('Success', __('backend.Subadmin_deactive_sucessfully'));
         return response()->json(['success' => 'true']);
    }
    ...
    public function status_inactive(Request $request){
        ...
        Alert::success('Success', __('backend.Subadmin_active_sucessfully'));
         return response()->json(['success' => 'true']);
    }
    ...
    {   
        ...
        $totalAr = User::where('status','!=','2')->where('user_type',3);
