...
class StoreController extends Controller
{
    ...
    public function index()
    {
        ...
        return view("dashboard.store.list");
    }

    public function create()
    {
        $categories = Categories::where('status',1)->orderby('title','ASC')->get();
        $countryData = Country::where('status','!=',2)->orderBy('phonecode','ASC')->get();    
        $settings = Setting::find(1);
        return view("dashboard.store.create",compact('categories','settings','countryData'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $validator =Validator::make($request->all(), [
            ...
            'store_name_fr' => 'required|max:60',
            'address' => 'required',
            'contact_number' => 'required|regex:/^[0-9]{8,15}$/'
        ],
        [            
            'store_name_fr.required'=> "The store name fr field is required.",
            'store_name_fr.max'=> "The store name should not be greater than 60 characters.",
            'store_name.max'=> "The store name fr should not be greater than 60 characters.",
            'contact_number.required' => "Phone number field is required",            
            'contact_number.digits_between' => "Phone number field should appear 8 to 15 digits"
        ]
    ); 
    if ($validator->fails()) {
    ...
    // }
          ...
          $storeDetails->uniqid = $uniqid;
          $storeDetails->sub_admin_type = Auth::user()->user_type;
          $storeDetails->sub_admin_id = Auth::user()->id;
          $storeDetails->store_name = $request->store_name;
          $storeDetails->store_name_fr = $request->store_name_fr;
          $storeDetails->address = $request->address;
          $storeDetails->phone_code = $request->phone_code;
          $storeDetails->contact_number = $request->contact_number;
          $storeDetails->latitude = $request->property_latitude;
          $storeDetails->longitude = $request->property_longitude;
          $storeDetails->status = 1;
          $storeDetails->save();
       
            //dd('sdf');
            //  $week_days= DB::table('week_list')->pluck('week_name');
            //        dd($week_days);
        

          $mondayTiming = new StoreTimingWeek();
          $mondayTiming->store_id = $storeDetails->id;
          ...
          $tuesdayTiming->store_id = $storeDetails->id;
          ...
          $wednesdayTiming->store_id = $storeDetails->id;
          ...
          $thursdayTiming->store_id = $storeDetails->id;
          ...
          $fridayTiming->store_id = $storeDetails->id;
          ...
          $saturdayTiming->store_id = $storeDetails->id;
          ...
          $sundayTiming->store_id = $storeDetails->id;
        ...
        //   Alert::success('Success', __('Store create successfully.'));

          return redirect()->route('store')->with('success', 'Store create successfully.');
        // Alert::success('Success', __('Store create successfully.'));
    ...
    public function validateRequest($id="")
    {
        if($id !="")
        {
            ...
            [
                'store_name.max'=> "The store name should not be greater than 60 characters.",
                'store_name_fr.required'=> "The store name fr field is required.",
                'store_name_fr.max'=> "The store name fr should not be greater than 60 characters.",
                'contact_number.required' => "Phone number field is required",            
                'contact_number.digits_between' => "Phone number field should appear 8 to 15 digits"
            ]
        );
    ...
    public function edit($id)
    {
            ...
            $countryData = Country::where('status','!=',2)->orderBy('phonecode','ASC')->get(); 
            $StoreTimingWeek = DB::table('store_timing_week')->leftjoin('week_list','week_list.id','=','store_timing_week.week_id')->select('store_timing_week.*','week_list.name','week_list.week_name')->where('store_timing_week.store_id',$store_id)->get();

            return view('dashboard.store.edit', compact('StoreDetails','StoreTimingWeek','countryData'));
    }
    ...
    public function update(Request $request,$id)
    {
        ...
        $product = StoreDetails::where('id', $store_id)->update(array(
                ...
                'store_name_fr' => $request->store_name_fr,
        ...
        return redirect()->route('store')->with('successMessage', 'Store updated successfully.');
    }
    ...
    public function show($id)
    {
        ...
        return view('dashboard.store.show', compact('StoreDetails','StoreTimingWeek'));
    }
    ...
    {   
        ...
        $totalAr = StoreDetails::where('status','!=',2);               
        if ($searchValue!="") {
            $totalAr = $totalAr->where(function ($query) use ($searchValue) {
                 $query->orWhere('store_details.store_name', 'like', '%' . $searchValue . '%');
                 $query->orWhere('store_details.address', 'like', '%' . $searchValue . '%');
                 $query->orWhere('store_details.phone_code', 'like', '%' . $searchValue . '%');
                 $query->orWhere('store_details.contact_number', 'like', '%' . $searchValue . '%');
            });
        }
        ...
        {
            $categoryShow =  route('store.show',['id'=>$data->id]);
            $categpryEdit =  route('store.edit',['id'=>$data->id]);
            ...
            $phone_code = ($data->phone_code)?('+'.$data->phone_code.'  '.$data->contact_number):'';
            ...
            $data_arr[] =array(
              ...
              "contact_number" =>  isset($phone_code) ? $phone_code : '-',
