...
{{-- product Listing --}}
@if(!$has_offer_products && !$has_bogo_products)
    <div class="text-center py-5">
        <div class="content-text">
            <div class="error-page largest-text">
                <div class="image-center" style="margin-right:55px;">
                    <img width="380px" src="{{ asset('assets/dashboard/images/error.png') }}" alt="offer-not-found">
                </div>
        ...
        <div class="row" id = "product_list">
            <div class="col-12">
                ...
                <ul class="nav nav-tabs mb-4" id="specialOfferTabs" role="tablist">
                    ...
                    @if($has_bogo_products)
                    <li class="nav-item" role="presentation">
                        <button class="nav-link {{ !$has_offer_products ? 'active' : '' }}" id="bogo-tab" data-bs-toggle="tab"
                ...
                <div class="short-by">
                    ...
                    id="total_offer_page_count">{{ @$offer_total_product_count ?: '0' }}</span> Items )</p> --}}
                    <p class="mb-0">
                        {{ Helper::language('all_products') }}
                        (
                        <span id="total_offer_page_count">
                            {{ $offer_total_product_count ?: 0 }}
                        </span>
                        <span id="total_bogo_page_count" style="display: none;">
                            {{ $bogo_total_product_count ?: 0 }}
                        </span>
                        Items )
                    </p>
                ...
                <div class="tab-content" id="specialOfferTabContent">
                    @if($has_offer_products)
                        <div class="tab-pane fade show active" id="offer" role="tabpanel" aria-labelledby="offer-tab">
                            <div class="row product-listing-row" id="offer_table_data">
                                @include('frontend.product.ajax_filter_offer', [
                            ...
                            <div class="row">
                                <div class="offset-md-5 col-md-7">
                                    <div class="show-more">
                                        <button
                                            @if ($offer_total_product_count != 0 && $offer_total_product_count > 16) style="display:block;" @else style="display:none;" @endif
                                            class="solid-button show-more load-more-offer-data">{{ Helper::language('show_more') }}</button>
                                        @if ($offer_total_product_count != 0)
                                            <p id="offer_showing">{{ Helper::language('showing') }}
                                                <span id="p-offer-current-count">{{ $offer_showing_product_count ?: 0 }} </span> of <span
                                                    id="p-offer-total-count">{{ $offer_total_product_count }}</span>
                                            </p>
                                        @endif
                                    </div>
                                </div>
                    ...
                    @if($has_bogo_products)
                    <div class="tab-pane fade {{ !$has_offer_products ? 'show active' : '' }}" id="bogo" role="tabpanel"
                        aria-labelledby="bogo-tab">
                        <div class="row product-listing-row" id="bogo_table_data">
                            @include('frontend.product.ajax_filter_bogo', [
                        ...
                        <div class="row">
                            <div class="offset-md-5 col-md-7">
                                <div class="show-more">
                                    @if ($bogo_total_product_count > 16)
                                        <button class="solid-button show-more load-more-bogo-data">{{ Helper::language('show_more') }}</button>
                                    @endif

                                    @if ($bogo_total_product_count != 0)
                                        <p id="bogo_showing">{{ Helper::language('showing') }}
                                            <span id="p-bogo-current-count">{{ $bogo_showing_product_count ?: 0 }} </span> of <span
                                                id="p-bogo-total-count">{{ $bogo_total_product_count }}</span>
                                        </p>
                                    @endif
                                </div>
                            </div>
...
<script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $(".load-more-offer-data").click(function() {
                ...
                $.ajax({
                    url: "{{ route('loadfilterofferlist') }}?sort_by=" + sort_by + "&brand_ids=" +
            ...
            $(".load-more-bogo-data").click(function() {
                ...
                $.ajax({
                    url: "{{ route('loadfilterbogolist') }}?sort_by=" + sort_by + "&brand_ids=" +
                        brand_ids + "&category_ids=" + category_ids + "&subcategory_ids=" +
                        subcategory_ids + "&min_price=" + min_price + "&max_price=" + max_price +
                        '&product_last_id=' + product_last_id + "&current_page_count=" +
                        current_page_count,
